...
export function initEventListeners() {
  ...
  document.getElementById('dateRangeMode').addEventListener('change', toggler.toggleRangeMode);
  
  // 其他原有的事件监听器
  document.getElementById('closeModal').addEventListener('click', toggler.closeModal);
  ...
  document.querySelector('.paper-modal').addEventListener('click', (event) => {
    ...
    if (event.target === modal) {
      ...
      if (pdfContainer && pdfContainer.classList.contains('expanded')) {
        ...
        if (expandButton) {
          toggler.togglePdfSize(expandButton);
        }
        // 阻止事件继续传播，防止关闭整个模态框
    ...
    // 左右箭头键导航论文（仅在论文模态框打开时）
      ...
      if (paperModal.classList.contains('active')) {
        ...
        if (event.key === 'ArrowLeft') {
          toggler.navigateToPreviousPaper();
        } else if (event.key === 'ArrowRight') {
          toggler.navigateToNextPaper();
        }
      }
    }
  });
