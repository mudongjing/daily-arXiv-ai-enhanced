...
if __name__ == '__main__':
    ...
    #     # ],
    #     messages = [
    #         {"role": "system", "content": "你作为一个作为一个学术能力很强的学者，对数学，计算机科学等领域有深入的研究和理解，下面我将给出arxiv网站上论文的摘要内容，你根据内容，首先判断该论文是否是纯理论性的主题，给出简单的是或不是的回答，然后给出具有学术性的简单明了的关键词，用逗号分隔，全部使用中文回答，判断与关键词各占一行，中间不要有其他额外信息，所有回答之间也不要有额外信息。"},
    #         {"role": "user", "content": "The generalised Wronskian of differential order k⩾1 for N functions f1, …, fN in d⩾1 independent variables x1, …, xd is the determinant of the matrix with these functions' derivatives ∂|σi|fj/∂(x1)σ1i⋯∂(xd)σdi (of orders 0⩽|σi|⩽k), where the multi-indices σi mark (all or part of) fibre variables uσi in the kth jet space Jk(ℝd→ℝ). We prove that these (in)complete Wronskians -- provided that their lowest-order parts are complete at differential orders ℓ⩽1 -- over the d-dimensional base satisfy the table of bi-linear, Jacobi-type identities for Schlessinger--Stasheff's strongly homotopy Lie algebras."},
    #         {"role": "user", "content": "We study the monoid algebra nm of semistandard Young tableaux, which coincides with the Gelfand--Tsetlin semigroup ring n when m=n. Among others, we show that this algebra is commutative, Noetherian, reduced, Koszul, and Cohen--Macaulay. We provide a complete classification of its maximal ideals and compute the topology of its maximal spectrum. Furthermore, we classify its irreducible modules and provide a faithful semisimple representation. We also establish that its associated variety coincides with a toric degeneration of certain partial flag varieties constructed by Gonciulea--Lakshmibai. As an application, we show that this algebra yields injective embeddings of 𝔰𝔩n-crystals, extending a result of Bossinger--Torres."},
    #     ],
    # )
